#!/bin/bash
# This script sets up a docker-compose installation of the reverse-proxy of a Robinhood clone on EC2

pathToPEM=$1
instance=$2
install=${3:-0} # defaults to 0

# Create the reverse-proxy directory if it doesn't exist
ssh -i $pathToPEM ec2-user@$instance 'mkdir -p reverse-proxy'

# Moves these files onto that instance
scp -i $pathToPEM ./docker-compose.yml .env ec2-user@$instance:~/reverse-proxy
